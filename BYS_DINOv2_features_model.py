...
def run_deep_model_14():
    ...
    # 🎯 读取 14 维的主成分表
    df_features = pd.read_csv("PCA_14_dinov2.csv")
    ...
    print("\n================ 阶段 5: 全方位保存数据与图表 ================")
    save_dir = "BYS_DINOv2_model_14D_result"
    os.makedirs(save_dir, exist_ok=True)

    summary.to_csv(f"{save_dir}/DINOv2_14D_Model_Summary.csv")
    with open(f"{save_dir}/DINOv2_14D_Model_Metrics_Report.txt", "w", encoding="utf-8") as f:
        f.write(metrics_text)

    results.to_netcdf(f"{save_dir}/DINOv2_14d_model_trace.nc")
    ...
    ax.errorbar(means, y_pos, xerr=[means - hdi_lower, hdi_upper - means],
                fmt='o', color='#ff7f0e', ecolor='#ff7f0e', elinewidth=2, capsize=4)
    ...
    ax.set_title('Impact of 14 DINOv2 Deep PCs on Aesthetics', weight='bold', pad=15)
    sns.despine()
    plt.grid(axis='x', linestyle='--', alpha=0.5)
    plt.tight_layout()
    plt.savefig(f'{save_dir}/DINOv2_14D_Fig1_Forest_Plot.png', dpi=300, bbox_inches='tight')
    plt.close()
