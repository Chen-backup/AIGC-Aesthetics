...
RATINGS_PATH = ROOT / "ratings_for_bayesian_model.xlsx"
MAPPING_PATH = ROOT / "renumber&gender.xlsx"

FEATURE_SETS = {
    "DINOv2": ROOT / "PCA_14_dinov2.csv",
    "InsightFace": ROOT / "PCA_14_features.csv",
    "StyleGAN": ROOT / "PCA_14_stylegan_w.csv",
}
...
def infer_subject_id(image_name: str) -> str:
    ...
    face_match = re.match(r"^(face_\d+)", stem)
    if face_match:
        return face_match.group(1)
    ugly_match = re.match(r"^(ugly\d+)_\d+$", stem)
    if ugly_match:
        return ugly_match.group(1)
    return stem
...
def load_image_level_dataset(model_name: str, feature_path: Path) -> pd.DataFrame:
    ...
    mapping["face_id"] = mapping["face_id"].astype(str).str.strip()
    ...
    rating_summary = (
        ...
        .rename(columns={"image": "Number"})
    )

    mapped_features = features.merge(
        mapping[["face_id", "Number"]],
        left_on="image_name",
        right_on="face_id",
        how="inner",
        validate="one_to_one",
    )
    df = mapped_features.merge(rating_summary, on="Number", how="inner", validate="one_to_one")
    ...
    return df[cols].sort_values("Number").reset_index(drop=True)
...
def run_cv(df: pd.DataFrame, model_name: str, cv_type: str):
    ...
    for fold_idx, train_idx, test_idx in folds:
        ...
        test_df = df.iloc[test_idx][["Number", "image_name", "subject_id", "rating_mean"]].copy()
...
def write_report(summary: pd.DataFrame, fold_results: pd.DataFrame, dataset_info: pd.DataFrame):
    lines = [
        "Face black-box feature Ridge 5-fold CV",
        "=" * 48,
        "",
        "Task:",
        "  Predict image-level mean aesthetic rating from 14 PCA features.",
        "",
        "Model:",
        ...
        "",
        "CV paradigms:",
        "  image-level: shuffled KFold over individual images.",
        "  subject-level: held-out subject IDs; all images from test subjects are excluded from training.",
        "",
        "Dataset summary:",
    ]
    ...
    for _, row in fold_results.sort_values(["model_name", "cv_type", "fold"]).iterrows():
        lines.append(
            ...
            f"test subjects={int(row['n_test_subjects'])}"
        )

    (OUT_DIR / "Face_BlackBox_Ridge_CV_Report.txt").write_text("\n".join(lines), encoding="utf-8")
...
def main():
    ...
    for model_name, feature_path in FEATURE_SETS.items():
        ...
        print(
            f"Data: {len(df)} images, {df['subject_id'].nunique()} subjects, "
    ...
    fold_results.to_csv(OUT_DIR / "Face_BlackBox_Ridge_CV_Fold_Results.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(OUT_DIR / "Face_BlackBox_Ridge_CV_Summary.csv", index=False, encoding="utf-8-sig")
    predictions.to_csv(OUT_DIR / "Face_BlackBox_Ridge_CV_Predictions.csv", index=False, encoding="utf-8-sig")
    image_dataset.to_csv(OUT_DIR / "Face_BlackBox_Ridge_CV_Image_Dataset.csv", index=False, encoding="utf-8-sig")
    dataset_info_df.to_csv(OUT_DIR / "Face_BlackBox_Ridge_CV_Dataset_Info.csv", index=False, encoding="utf-8-sig")
