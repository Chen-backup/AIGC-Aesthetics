...
def run_landscape_places365_model_10() -> None:
    print("================ Stage 1: Load PCA-10 Places365 features and merge tables ================")
    base_dir = Path(__file__).resolve().parent
    ratings_path = base_dir.parent / "ratings_for_bayesian_model.csv"
    features_path = base_dir / "PCA_10_places365.csv"
    ...
    metrics_text = (
        "========== Landscape Places365 PCA-10 Model Metrics ==========\n\n"
        ...
        f"   - Places365 PCA-10 marginal R^2: {marginal_r2_str}\n"
    )
    print("\n" + metrics_text)

    print("\n================ Stage 5: Save outputs and figures ================")
    save_dir.mkdir(parents=True, exist_ok=True)
    summary.to_csv(save_dir / "Places365_10D_Model_Summary.csv")
    with open(save_dir / "Places365_10D_Model_Metrics_Report.txt", "w", encoding="utf-8") as f:
        f.write(metrics_text)
    results.to_netcdf(save_dir / "Places365_10d_model_trace.nc")
    ...
    ax.set_title("Impact of 10 Places365 PCs on Landscape Aesthetic Ratings", weight="bold", pad=15)
    sns.despine()
    plt.grid(axis="x", linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig(save_dir / "Places365_10D_Fig1_Forest_Plot.png", dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    freeze_support()
    run_landscape_places365_model_10()
