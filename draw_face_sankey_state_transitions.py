...
PNG_OUTPUT = RESULT_DIR / "Face_Alluvial_State_Transitions_14Features.png"
PNG_OUTPUT_TEXTLESS = RESULT_DIR / "Face_Alluvial_State_Transitions_14Features_textless.png"
PNG_OUTPUT_FULL_RANGE = RESULT_DIR / "Face_Alluvial_State_Transitions_14Features_0_100.png"
PNG_OUTPUT_FULL_RANGE_TEXTLESS = RESULT_DIR / "Face_Alluvial_State_Transitions_14Features_0_100_textless.png"
DATA_OUTPUT = RESULT_DIR / "Face_Alluvial_State_Transitions_14Features_Data.csv"
...
def compute_flows(state_df: pd.DataFrame, node_rects: dict) -> tuple[list, pd.DataFrame]:
    ...
    for i in range(len(FEATURE_ORDER) - 1):
        ...
        pair_counts["_source_order"] = pair_counts[source_feature].map(state_rank)
        pair_counts["_target_order"] = pair_counts[target_feature].map(state_rank)
...
def draw_flow(ax: plt.Axes, flow: dict) -> None:
    ...
    patch.set_edgecolor((*mpl.colors.to_rgba(color)[:3], 0.05))
    ax.add_patch(patch)
...
def draw_column_height_guides(ax: plt.Axes, column_heights: pd.Series) -> None:
    for i, feature in enumerate(FEATURE_ORDER):
        ...
        y0 = 0.5 - height / 2
        y1 = 0.5 + height / 2
        ax.vlines(i * X_SPACING, y0, y1, color="#20242A", lw=0.6, alpha=0.16, zorder=0)
...
def draw_sankey(
    ...
    ax.set_title(
        "Alluvial transitions of rater preference states across face features",
        fontsize=17,
        fontweight="bold",
        pad=20,
    )
    ...
    fig.subplots_adjust(left=0.02, right=0.992, bottom=0.13, top=0.86)
