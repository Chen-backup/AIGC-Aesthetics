...
RAW_FEATURES_PATH = ROOT_DIR / "dinov2_features.csv"
MODEL_OUTPUT_DIR = ROOT_DIR / "BYS_Fusion_28D_DINOv2_result"
...
def main() -> None:
    ...
    csv_path = ROOT_DIR / f"PCA_{n_components}_dinov2.csv"
    plot_path = ROOT_DIR / f"DINOv2_PCA_{n_components}_Variance.png"
    MODEL_OUTPUT_DIR.mkdir(exist_ok=True)
    pkl_path = MODEL_OUTPUT_DIR / f"dinov2_pca_{n_components}d.pkl"
    ...
    plt.axhline(args.target_variance, color="#2a9d8f", linestyle="--", label="Target Variance")
    plt.title(
        f"DINOv2 Explained Variance: 768D to {n_components}D",
        fontsize=16,
        weight="bold",
    )
    plt.xlabel("Principal Component (PC)", fontsize=14)
    plt.ylabel("Explained Variance Ratio", fontsize=14)
    plt.xticks(range(1, n_components + 1, max(1, n_components // 12)))
    plt.legend(loc="best")
