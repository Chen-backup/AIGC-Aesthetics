...
def plot_major_analysis(stats, output_path):
    """Plot major analysis."""
    plt.figure(figsize=(14, 8))
    
    # Define colors
    colors = ['#9EB5FF', '#FF9E9E', '#87CEEB', '#FFB6C1', '#90EE90', '#FFD700', '#9370DB', '#8FBC8F', '#FFA07A', '#20B2AA']
    ...
    plt.title('Effect of Major Category on Aesthetic Judgment', fontsize=16, fontweight='bold')
    plt.ylim(0, 5)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f'Major analysis plot saved: {output_path}')

def plot_reaction_time_comparison(stats, output_path):
    """Plot reaction time comparison by major category."""
    ...
    plt.title('Reaction Time by Major Category', fontsize=16, fontweight='bold')
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_score_distribution(stats, title, output_path):
    """Plot score distribution by major category."""
    plt.figure(figsize=(14, 8))
    ...
    for i, (group, stat) in enumerate(stats.items()):
        ...
        plt.bar(score_dist.index - 0.5 + i * 0.15, 
                score_dist.values, width=0.12, label=group, color=colors[i % len(colors)])
...
def plot_block_major_analysis(block_stats, output_path):
    """Plot major analysis by block type."""
    ...
    for i, block in enumerate(blocks):
        ...
        plt.title(f'{block.capitalize()} Images: Score by Major Category')
        plt.ylim(0, 5)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
    
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f'Block major analysis plot saved: {output_path}')
...
def generate_report(major_stats, block_stats, anova_result, output_path):
    ...
    # Analyze mean scores by major category
...
def main():
    ...
    # Generate plots
    plot_major_analysis(major_stats, 'code/picture/major_analysis.png')
    plot_reaction_time_comparison(major_stats, 'code/picture/major_reaction_time.png')
    plot_score_distribution(major_stats, 'Score Distribution by Major Category', 'code/picture/major_score_distribution.png')
    plot_block_major_analysis(block_stats, 'code/picture/major_block_analysis.png')
