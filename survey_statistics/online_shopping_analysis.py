...
def categorize_shopping_frequency(frequency):
    ...
    frequency_map = {
        ...
        '每月3-4次': 'Monthly (3-4 times)',
        '每周1-2次': 'Weekly (1-2 times)',
...
def analyze_shopping_effect(df):
    ...
    # 按网购频率分组分析
    shopping_stats = df.groupby('shopping_category').agg({
...
def plot_shopping_analysis(df):
    """绘制网购频率分析图表"""
    if df.empty:
        return
    
    # 按网购频率分组计算平均评分和反应时间
    shopping_groups = df.groupby('shopping_category').agg({
        'score': 'mean',
    ...
    shopping_score = df.groupby('shopping_category')['score'].mean().sort_values(ascending=False)
    shopping_score.plot(kind='bar', color='skyblue')
    plt.title('平均评分 by 网购频率')
    plt.xlabel('网购频率')
    plt.ylabel('平均评分')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('code/picture/shopping_analysis.png')
    print("Shopping analysis plot saved: code/picture/shopping_analysis.png")
    
    # 反应时间分析
    plt.figure(figsize=(12, 6))
    shopping_rt = df.groupby('shopping_category')['reaction_time_ms'].mean().sort_values()
    shopping_rt.plot(kind='bar', color='lightgreen')
    plt.title('平均反应时间 by 网购频率')
    plt.xlabel('网购频率')
    plt.ylabel('平均反应时间 (ms)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('code/picture/shopping_reaction_time.png')
    print("Reaction time comparison plot saved: code/picture/shopping_reaction_time.png")
    ...
    plt.title('评分分布 by 网购频率')
    plt.xlabel('评分')
    plt.ylabel('频数')
    plt.legend()
    plt.tight_layout()
    plt.savefig('code/picture/shopping_score_distribution.png')
    print("Score distribution plot saved: code/picture/shopping_score_distribution.png")
    
    # 按图像类型分析
    plt.figure(figsize=(14, 8))
    block_shopping = df.groupby(['block', 'shopping_category'])['score'].mean().unstack()
    block_shopping.plot(kind='bar', ax=plt.gca())
    plt.title('不同图像类型的评分 by 网购频率')
    plt.xlabel('图像类型')
    plt.ylabel('平均评分')
    plt.xticks(rotation=45)
    plt.legend(title='网购频率')
    plt.tight_layout()
    plt.savefig('code/picture/shopping_block_analysis.png')
    print("Block shopping analysis plot saved: code/picture/shopping_block_analysis.png")
